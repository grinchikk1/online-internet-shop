  ...
  return (
    ...
    >
      ...
      >
          ...
          <Typography sx={{fontSize: "13px", fontWeight: "400", color: "#707070" }}>To leave a review, you need to sign up on the site. Required fields are marked *</Typography>
      <Formik
        initialValues={{
      ...
      >
 <Typography variant="h6" sx={{ textAlign: "left", color: "#A18A68" }}>Reviews({reviews.length})</Typography>
          ...
          <React.Fragment key={index}>
            ...
            <Review name={review.customer.firstName} date={review?.someCustomParam?.date} review={review.content} rating={review?.someCustomParam?.rating}
                ...
                
              {
                ...
                if (user && (user.isAdmin || user._id === review.customer._id)) {
                  dispatch(deleteReview(review._id))
                  
                } 
                else {setOpenDel(true) }
              }} />
            {index !== reviews.length - 1 && <Divider sx={{ width: "auto", marginBottom: "40px" }} />}
          </React.Fragment>
        ))}
